...
app.post("/verify-soulmark", (req, res) => {
  try {
    ...
    const soulmark =
      "0x" +
      crypto.createHash("sha256").update(email).digest("hex").substring(0, 32).toUpperCase();
...
app.post("/register", (req, res) => {
  try {
    ...
    const entry = { name, email, username, role, soulmark, createdAt: new Date().toISOString() };
    data.push(entry);
    fs.writeFileSync(registryPath, JSON.stringify(data, null, 2));
